...
C     created:   $Date: 2004-07-29 12:49:51 -0400 (Thu, 29 Jul 2004) $


c *******************************************************************
c     THIS VERSION OF DISORT HAS BEEN MODIFIED TO BE COMPATIBLE WITH 
c     RRTM, AER'S RAPID LONGWAVE RADIATIVE TRANSFER MODEL.  
c     CHANGES HAVE BEEN MARKED WITH A COMMENT LABELED "caer".
c
c     THIS VERSION HAS ALSO BEEN MODIFIED TO BE CALL D1MACH
c     INSTEAD OF R1MACH, AND IS INTENDED TO BE COMPILED IN
c     DOUBLE PRECISION.
c *******************************************************************
...
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     ...
     &                   DTAUC, SSALB, NMOM, PMOM, TEMPER, WVNMLO,
...
c     .. Parameters ..
     ...
     &          MXSQT
caer  Increased these parameter values
...
caer Added this variable to account for the Planck fractions noted
c    in RRTM
      
      REAL          fracsrev( maxcly)
c     ..
c     .. Local Scalars ..
...
caer In order to pass the self-tests, let PASS1 go without including
c    effect of Planck fraction
         ...
         if (pass1) then
            ...
            TPLANK = fracsrev(1)*
     &           TEMIS*PLKAVG( WVNMLO, WVNMHI, TTEMP )
            BPLANK = fracsrev(nlyr)*
     &           PLKAVG( WVNMLO, WVNMHI, BTEMP )
         endif
...
c    effect of Planck fraction
               if (pass1) then
                  ...
                  pkag_t2 = pkag(lc-1)*fracsrev(lc)
                  pkag_t0 = pkag(lc)*fracsrev(lc)
               endif
...
c               IF( DTAUCP( LC ).GT.0.0 ) XR1( LC ) =
c     &             ( PKAG_t0 - PKAG_t2 ) / DTAUCP( LC )
c               XR0( LC ) = PKAG_t2 - XR1( LC )*TAUCPR( LC-1 )

               if (dtaucp(lc) .lt. 1.e-6) then
                  xr0(lc) = pkag_t2
               else
                   XR1( LC ) =
     &             ( PKAG_t0 - PKAG_t2 ) / DTAUCP( LC )
...
caer Tailored routine for specular reflection
      call qgausn(nn,tmpc,tmpwt)
...
c               DO 40 K = 1, NMUG
c                  SUM  = SUM + GWT( K ) *
c     &                   BDREF( ALBEDO, CMU(IQ), CMU(JQ),
c     &                          PI*GMU(K) ) * COS( MAZIM*PI*GMU( K ) )
c   40          CONTINUE

c               BDR( IQ, JQ ) = 0.5 * ( 2. - DELM0 ) * SUM

               BDR( IQ, IQ ) = ALBEDO/(2.*cmu(iq)*tmpwt(iq))
...
c                       ** Loop over angle of reflection
         DO 20 K = 1, NMUG / 2
            SUM  = SUM + GWT( K )*GMU( K )*
     &             BDREF( ALBEDO, GMU( K ), MU, PI*GMU( JG ) )
   20    CONTINUE
...
c               can replace the D1MACH calls.
c
c   Called by- DISORT
c   Calls- D1MACH
c +-------------------------------------------------------------------+
...
c     Supplies surface bi-directional reflectivity.  Specular reflection
c     is assumed.
c
c
c
...
c      CALL ERRMSG( 'BDREF--Please supply a surface BDRF model', .TRUE. )
